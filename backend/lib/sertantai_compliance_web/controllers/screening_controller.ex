defmodule SertantaiComplianceWeb.ScreeningController do
  @moduledoc """
  ...
  def trigger_sync(conn, _params) do
    ...
    case Repo.query(
      ...
      {:ok, %{rows: [[config_id]]}} ->
        ...
        case SertantaiCompliance.Sync.Engine.run(config_uuid) do
          {:ok, result} ->
  ...
  def vocabulary(conn, _params) do
    # Actor labels from legal_register JSONB fields (see #132 for ActorDictionary table refactor)
    governed_actors = actor_labels_from_db("duty_holder", "responsibility_holder")
    government_actors = actor_labels_from_db("power_holder")

    # Fitness entities from v0.3 corpus (GIN-indexed)
    alias SertantaiCompliance.Fitness.EntityIndex
  ...
  def compliance_metrics(conn, _params) do
    ...
    metrics =
      SertantaiCompliance.Sync.Templates.ComplianceMetrics.get_metrics(org_id)
  ...
  defp actor_labels_from_db(columns) when is_list(columns) do
    unions =
      columns
      |> Enum.map(fn col ->
        "SELECT jsonb_array_elements_text(#{col}->'values') AS val FROM legal_register WHERE #{col} IS NOT NULL"
      end)
      |> Enum.join(" UNION ")

    query = "SELECT DISTINCT val FROM (#{unions}) sub ORDER BY val"
