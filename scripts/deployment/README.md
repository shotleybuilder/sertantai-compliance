...
- Includes migrations via `SertantaiCompliance.Release.migrate/0`
...
```bash
IMAGE_NAME="ghcr.io/YOUR_GITHUB_ORG/sertantai-compliance-backend"
```
...
Update `IMAGE_NAME` in script with your GitHub org/user:
```bash
IMAGE_NAME="ghcr.io/YOUR_GITHUB_ORG/sertantai-compliance-frontend"
```
...
2. **Update infrastructure docker-compose.yml:**
   ```yaml
   services:
     sertantai-compliance-backend:
       image: ghcr.io/YOUR_ORG/sertantai-compliance-backend:latest
       environment:
         DATABASE_URL: postgresql://postgres:password@postgres:5432/sertantai_compliance_prod
     ...
     sertantai-compliance-frontend:
       image: ghcr.io/YOUR_ORG/sertantai-compliance-frontend:latest
       environment:
...
3. **Pull and restart on production server:**
   ...
   docker compose pull sertantai-compliance-backend sertantai-compliance-frontend
   docker compose up -d sertantai-compliance-backend sertantai-compliance-frontend
   docker compose logs -f sertantai-compliance-backend
   ```
...
4. **Configure Nginx reverse proxy:**
   ...
   server {
       ...
       location / {
           proxy_pass http://sertantai-compliance-backend:4000;
       }
   }
   ...
   server {
       ...
       location / {
           proxy_pass http://sertantai-compliance-frontend:3000;
       }
   }
   ```
...
# Verify images exist
docker images | grep sertantai-compliance
...
```bash
# Check logs
docker logs sertantai-compliance-backend
...
```bash
# Check logs
docker logs sertantai-compliance-frontend
...
- Expected: `{"status": "ok", "service": "sertantai-compliance", "timestamp": "..."}`
