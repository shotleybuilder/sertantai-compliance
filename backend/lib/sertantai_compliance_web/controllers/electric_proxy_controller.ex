defmodule SertantaiComplianceWeb.ElectricProxyController do
  @moduledoc """
  ...
  @public_tables ~w(legal_register legal_register_uk legal_register_au legal_articles_uk legal_articles_au amendment_annotations legislative_definitions)
  ...
  defp validate_with_gatekeeper(params, auth_header) do
    auth_url = Application.get_env(:sertantai_compliance, :auth_url)
  ...
  defp maybe_add_secret(params) do
    case Application.get_env(:sertantai_compliance, :electric_secret) do
      nil -> params
  ...
  defp gatekeeper_plug do
    if Application.get_env(:sertantai_compliance, :test_mode, false) do
      {Req.Test, SertantaiComplianceWeb.GatekeeperClient}
    else
      nil
    end
  end
