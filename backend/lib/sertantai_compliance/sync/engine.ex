defmodule SertantaiCompliance.Sync.Engine do
  @moduledoc """
  ...
  alias SertantaiCompliance.Sync.{
    ActorTupleSync,
    Credentials,
    ProfileQuery,
    Providers.Baserow
  ...
  def clean(sync_config_id) do
    with {:ok, sync_config} <- load_sync_config(sync_config_id) do
      ...
      with {:ok, provider_config} <-
        ...
        SertantaiCompliance.Repo.query!(
  ...
  defp maybe_sync_controls(provider_config, lrt_rows, candidate_duties, sync_config, job) do
    ...
    if is_nil(controls_table_id) do
      ...
      with {:ok, controls} <-
             SertantaiCompliance.Legal.Control
             |> Ash.Query.for_read(:by_law_names, %{law_names: law_names})
             |> Ash.read(),
           {:ok, all_mappings} <-
             SertantaiCompliance.Legal.ControlMapping
             |> Ash.Query.for_read(:by_law_names, %{law_names: law_names})
             |> Ash.read() do
  ...
  defp maybe_sync_evidence_patterns(provider_config, lrt_rows, candidate_duties, sync_config, job) do
    ...
    if is_nil(ep_table_id) do
      ...
      with {:ok, evidence_patterns} <-
             SertantaiCompliance.Legal.EvidencePattern
             |> Ash.Query.for_read(:by_law_names, %{law_names: law_names})
             |> Ash.read(),
           {:ok, all_mappings} <-
             SertantaiCompliance.Legal.ControlMapping
      ...
      with {:ok, evidence_patterns} <-
        ...
        {:ok, artefact_templates} =
          if pattern_ids == [] do
            ...
            SertantaiCompliance.Legal.ArtefactTemplate
  ...
  defp load_sync_config(id) do
    case Ash.get(SertantaiCompliance.Sync.SyncConfiguration, id) do
      {:ok, config} -> {:ok, config}
  ...
  defp load_profile(id) do
    case Ash.get(SertantaiCompliance.Sync.SyncProfile, id) do
  ...
  defp load_entitlement(organization_id) do
    case SertantaiCompliance.Sync.OrgEntitlement
  ...
  defp provider_module(:baserow), do: SertantaiCompliance.Sync.Providers.Baserow
  ...
  defp create_job(sync_config) do
    SertantaiCompliance.Sync.SyncJob
