defmodule SertantaiCompliance.Sync.Providers.Baserow do
  @moduledoc """
  Baserow sync provider — pushes LRT/LAT data to Baserow (SaaS or self-hosted).

  Delegates all Baserow API calls to `SertantaiCompliance.Baserow.Client` and
  implements `SertantaiCompliance.Sync.ProviderBehaviour` with domain-specific
  ...
  @behaviour SertantaiCompliance.Sync.ProviderBehaviour

  require Logger

  alias SertantaiCompliance.Baserow.Client
  ...
  @family_options (SertantaiCompliance.Legal.FamilyLists.ehs_family() ++
                     SertantaiCompliance.Legal.FamilyLists.hr_family())
                  |> Enum.sort()
  ...
  # Actor vocabulary — queried from legal_register JSONB fields (no Zenoh dependency).
  ...
  defp holder_options do
    query = """
    SELECT DISTINCT val FROM (
      SELECT jsonb_array_elements_text(duty_holder->'values') AS val FROM legal_register WHERE duty_holder IS NOT NULL
      UNION
      SELECT jsonb_array_elements_text(responsibility_holder->'values') FROM legal_register WHERE responsibility_holder IS NOT NULL
      UNION
      SELECT jsonb_array_elements_text(rights_holder->'values') FROM legal_register WHERE rights_holder IS NOT NULL
      UNION
      SELECT jsonb_array_elements_text(power_holder->'values') FROM legal_register WHERE power_holder IS NOT NULL
    ) sub ORDER BY val
    """

    case SertantaiCompliance.Repo.query(query) do
  ...
  defp drrp_type_options do
    case SertantaiCompliance.Repo.query(
