...
│   ├── lib/
│   │   ├── sertantai_compliance/
│   │   │   ├── auth/              # User & Organization resources
│   │   │   ├── api.ex             # Ash Domain
│   │   │   ├── repo.ex            # Ecto Repo
│   │   │   └── application.ex     # OTP Application
│   │   ├── sertantai_compliance_web/
│   │   │   ├── controllers/
│   │   │   ├── endpoint.ex
│   │   │   └── router.ex
...
**Backend:**
- Module name: `SertantaiCompliance` → `YourApp`
- App name: `:sertantai_compliance` → `:your_app`
- Database: `sertantai_compliance_dev` → `your_app_dev`

**Frontend:**
- Package name: `sertantai-compliance-frontend` → `your-app-frontend`
- Display name: "Starter App" → "Your App"

**Files to rename:**
- `backend/lib/sertantai_compliance/` → `backend/lib/your_app/`
- `backend/lib/sertantai_compliance_web/` → `backend/lib/your_app_web/`

### 2. Add Your Domain Resources

The starter includes base Auth resources (User, Organization). Add your own:

1. Create your resources: `backend/lib/sertantai_compliance/your_domain/`
2. Add to domain: `backend/lib/sertantai_compliance/api.ex`
...
1. Configure JWT verification in `lib/sertantai_compliance_web/endpoint.ex`
...
**Backend** (`backend/.env`):
```bash
DATABASE_URL=postgresql://postgres:postgres@localhost:5435/sertantai_compliance_dev
...
Migrations run automatically on backend startup in production via `SertantaiCompliance.Release.migrate/0`.
...
docker exec -it your-backend-container /app/bin/sertantai_compliance eval "SertantaiCompliance.Release.migrate()"
```
...
# {"status": "ok", "service": "sertantai-compliance", "timestamp": "..."}
