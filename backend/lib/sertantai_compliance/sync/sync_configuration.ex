defmodule SertantaiCompliance.Sync.SyncConfiguration do
  @moduledoc """
  ...
  attributes do
    ...
    # Provider
    attribute :provider, SertantaiCompliance.Sync.Provider do
      allow_nil?(false)
    end
    ...
    # Schedule
    attribute :sync_frequency, SertantaiCompliance.Sync.SyncFrequency do
      allow_nil?(false)
    end

    # Change behaviour
    attribute :on_filter_change, SertantaiCompliance.Sync.ChangeBehaviour do
      allow_nil?(false)
      default(:delete)
      description("What to do when rows leave profile scope")
    end

    attribute :on_entitlement_change, SertantaiCompliance.Sync.ChangeBehaviour do
      allow_nil?(false)
      default(:delete)
    ...
    # State
    attribute :sync_status, SertantaiCompliance.Sync.SyncStatus do
      allow_nil?(false)
      default(:idle)
    end
  ...
  relationships do
    belongs_to :sync_profile, SertantaiCompliance.Sync.SyncProfile do
      source_attribute(:sync_profile_id)
      destination_attribute(:id)
      define_attribute?(false)
    end

    has_many :sync_jobs, SertantaiCompliance.Sync.SyncJob do
      source_attribute(:id)
      destination_attribute(:sync_configuration_id)
    end

    has_many :sync_row_mappings, SertantaiCompliance.Sync.SyncRowMapping do
      source_attribute(:id)
      destination_attribute(:sync_configuration_id)
    end
  end
